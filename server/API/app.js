...
app.get('/tshirt/:id', function(req, res, next) {
    dal.getItem(req.params.id, function(err, body) {
        if (err) {
            var getError = new HTTPError(404, 'Item Not Found', {
                error: err.message
            })
            next(getError)
        }
...
app.get('/tshirts', function(req, res, next) {
    ...
    dal.listItems('tshirts', limit, function(err, body) {
        if (err) {
            var listError = new HTTPError(500, 'Something went wrong on our end!', {
                error: err.message
            })
            next(listError)
        }
...
app.post('/tshirt', function(req, res, next) {
    dal.addItem(req.body, function(err, body) {
        if (err) {
            var addError = new HTTPError(400, 'Not able to post!', {
                error: err.message
            })
            next(addError)
        }
